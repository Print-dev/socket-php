...
while (true) {
  ...
  foreach ($read as $read_socket) {
    $data = @socket_read($read_socket, 2048); //LA DATA ES LO QUE LE ENVIAMOS DESDE NUESTRA VISTA 
    ...
    $registro = ws_decode($data);
    if (!empty($registro)) {
      $decoded = json_decode($registro, true);

      if ($decoded['type'] === 'registro') {
        $response = json_encode([
          'type' => 'registro',
          'mensaje' => 'Nuevo producto registrado',
          'datos' => $decoded['registrado']  
        ]);
        ...
        foreach ($clients as $client) {
          ...
          if (
            ...
            if (socket_write($client, ws_encode($response)) === false) {
              echo "Error al enviar mensaje a cliente $current_socket_id: " . socket_strerror(socket_last_error($client)) . "\n";
            }
          }
        }

        echo "Producto registrado y notificado a los clientes\n";
      }
    }
  }
